      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-5xl bg-[#050B2E] border border-white/10 rounded-[40px] shadow-3xl overflow-hidden"
          >
            ...
            <div className="p-8 lg:p-12">
              <div className="text-center mb-12">
                <Typography variant="h2" className="text-4xl uppercase tracking-tighter mb-4">Select Fleet Tier</Typography>
                <Typography variant="p" className="text-white/40 max-w-xl mx-auto uppercase tracking-widest text-[10px] font-black">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {plans.map((plan) => (
                  <Card 
                    key={plan.id}
                    variant="glass" 
                    className={`relative p-8 flex flex-col border-2 transition-all duration-500 hover:scale-[1.02] ${
                      currentPlan.includes(plan.name) ? "border-[#0075FF] bg-[#0075FF]/5" : "border-white/5 hover:border-white/20"
                    }`}
                  >
                    ...
                    <Button 
                      ...
                      className={`w-full h-12 rounded-xl font-black uppercase tracking-widest text-xs transition-all ${
                        currentPlan.includes(plan.name) ? "bg-white/5 text-white/20 border border-white/5" : "bg-white text-black hover:bg-neutral-200"
                      }`}
                    >
